    ...
    try:
        if str(message.channel.type) == "private":
            if message.author.bot == False:
                embed = Embed(title = f"문의를 하실건가요?",description = f"내용 : {message.content}",timestamp=message.created_at , color = random_color())
                try: 
...
class yes(ui.View):
    ...
    async def yes(self , button : Button , inter : Integration):
        embed = Embed(title = "문의가 왔어요!" , description=f"{inter.message.embeds[0].description.replace('내용 : ' , '')} \n\nid : {inter.user.id}\nname:{inter.user}" , color = random_color())
        try:
