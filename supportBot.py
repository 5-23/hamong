...
@client.slash_command(description="개발자만 사용 가능.")
...
class yes(ui.View):
    @ui.button(label="네" , style=ButtonStyle.green , emoji=":vv:")
    async def yes(self , button : Button , inter : Integration):
        ...
        await inter.response.send_message(embed = Embed(title = "문의가 완료 되었습니다." , color = random_color()))
    @ui.button(label="아니요." , style=ButtonStyle.red , emoji=":xx:")
